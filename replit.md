...
- **Admin Dashboard:** Comprehensive management hub featuring:
    ...
    - **Talent Profiles:** Talent profile database management with metrics, active/inactive tabs, detailed table columns, and action menus (View Document, Edit, Mark Inactive, Delete).
      - **Add New Profile:** 2-step wizard for adding talent profiles (similar to Partner module's Add Hiring Record flow):
        - **Step 1 - Basic Information:** Partner Organization (dropdown selection from 8 partner organizations), Name, Role (dropdown with 11 role options), Top Skills, Experience (Years + Months), Monthly Rate, Notice Period, Location
        - **Step 2 - Document Uploads:** Resume upload (mandatory), Aadhar Card upload (optional), PAN Card upload (optional), Degree Proof upload (optional) with file preview and remove functionality
